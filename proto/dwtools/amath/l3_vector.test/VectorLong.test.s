...
{
  test.case = 'src - empty vector, crange - 0';
  var src = vad.fromLongWithStride( [], 2 );
  var got = _.avector.review( src, 0 );
  var exp =  [];
  test.identical( got, exp );
  test.is( got !== src );
...
{
  test.case = 'src - empty vector, crange - 0';
  var src = vad.fromNumber( vad.from( [] ), 0 );
  var got = _.avector.review( src, 0 );
  var exp = [];
  test.identical( got, exp );
  test.is( got !== src );
  ...
  test.case = 'crange - 0';
  var src = vad.fromNumber( vad.from( [ 0, 1, 2, 3, 4, 5 ] ), 6 );
  var got = _.avector.review( src, 0 );
  var exp = [ 0, 1, 2, 3, 4, 5 ];
  test.identical( got, exp );
  test.is( got !== src );
...
function mapDstIsVector( test )
{
  ...
  test.case = 'src - vector, onEach - null';
  var dst = _.avector.make( [ -1, -2, -3, -4, -5 ] );
  var src = _.avector.make( [ 1, 2, 3, 4, 5 ] );
  var got = _.avector.map( dst, src, null );
  var exp = _.avector.make( [ -1, -2, -3, -4, -5 ]  );
  test.identical( got, exp );
  ...
  var src = _.avector.make( [ 1, 2, 3, 4, 5 ] );
  var got = _.avector.map( dst, src, ( e, k, s, d ) => d.eGet( k ) - s.eGet( k ) );
  var exp = _.avector.make( [ -2, -4, -6, -8, -10 ] );
  test.identical( got, exp );
  test.is( got !== src );
  test.is( got === dst );
