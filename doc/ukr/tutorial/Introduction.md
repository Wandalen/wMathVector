...
Перевірка `arrayIs()` повертає `true` лише для `array`. Перевірка `longIs()` повертає `true` для `array` та `buffer`. Але перевірка `vectorIs()` повертає `true` для всіх форматів задання вектора.
